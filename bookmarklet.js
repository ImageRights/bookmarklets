// Based on npm package `bookmarklet`, but with babel dependency removed
// and uglify-js replaced with uglify-es
...
function parseFile (data) {
  ...
  data.match(/[^\r\n]+/g).forEach(function (line, i, lines) {
    ...
    if (rComment.test(line)) {
      ...
      let canonicalComment = comment.toLowerCase().replace(/\s+/g, '')
        ...
        if (canonicalComment == closeMetadata.toLowerCase()) {
          ...
          if (m) {
            ...
            if (k) {
              ...
              console.warn(`ignoring invalid metadata option: '${k}'`)
            }
          }
        }
      }
